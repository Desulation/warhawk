...
function blankTextFieldValidator() {
  var form, formLength, numForms = document.forms.length;
  ...
  //now check if invalid characters
  //return val_illegal_char();
}
function reset_alert_count() {
  illegal_alert_count = 0;
}
//Function to check for illegal characters which will cause display issues on the PS2
function val_illegal_char() {
  /** RMS 070103 updated invalid list courtesy of Conrado- invalid: <>`%&~^
   * valid (unescaped): !#$"()*,-./:;=?@[]_{|}"+\;
   *    var INVALID_CHAR_REGEX = /[^\w\s!#$"()*,-./:;=?@\\[\\]_\\{|\\}\\\"+\\\\]/;
   *    var INVALID_CHAR_REGEX = /[^\w\s!#$"()*,-./:;=?@\[\]_{|}"+\\]/;
   * underscore can't be found at all whether it's in the pattern or not. Placed in the pattern just because.
   */
  reset_alert_count();
  var form, formLength, formElement, formElementValue,
      numForms = document.forms.length,
      illegal_alert_arr = new Array,
      illegal_char_arr = new
        Array(">", "<", "&", "`", "~", "^", "%"),
      illegal_alert_count = 0;
...
function maxTextAreaValidator(formName, textAreaName, maxSize) {
  var oTextArea, textAreaLength;
  //first check if blank
  if (!blankTextFieldValidator()) return;
...
function blankRadioValidator(formName) {
	var isSelected = false, formLength = form.length;
...
var NS4 = (navigator.appName == "Netscape") && (parseInt(navigator.appVersion.toString()) == 4),
    NS6 = (navigator.appName == "Netscape") && (parseInt(navigator.appVersion.toString()) >= 5),
